...
function loancalculate() {
  const loanAmount = jQuery("#emi-principal-show").text();
  const numberOfMonths = jQuery("#emi-totalyear-show").text() * 12;
  const rateOfInterest = jQuery("#emi-interest-show").text();
  ...
  const int_str = interest
    ...
    .replace(/,/g, "")
    .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  $input_principal = $("#emi-principal-input")
    .val()
    .toString()
    .replace(/,/g, "")
    .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
...
function amortization_calculator(loanAmount, emi, rate) {
  ...
  content +=
    '<div class="amortization-data year-outer"><i class="fas fa-plus-circle"></i><span>' +
    year_no +
    "</span></div>";
  ...
  while (current_balance > 0) {
    ...
    const principal_paid_str = principal_paid
      .toFixed(0)
      .toString()
      .replace(/,/g, "")
      .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    const toward_interest_str = toward_interest
      .toFixed(0)
      .toString()
      .replace(/,/g, "")
      .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    const emi_str = emi
      ...
      .replace(/,/g, "")
      .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    const current_balance_str = current_balance
      .toFixed(0)
      .toString()
      .replace(/,/g, "")
      .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    ...
    content +=
      '<div class="amortization-data year">' + payment_counter + "</div>";
    content +=
      '<div class="amortization-data principal">$<span>' +
      principal_paid_str +
      "</span></div>";
    content +=
      '<div class="amortization-data interest">$<span>' +
      toward_interest_str +
      "</span></div>";
    content +=
      '<div class="amortization-data total-payment">$<span>' +
      emi_str +
      "</span></div>";
    content +=
      '<div class="amortization-data balance">$<span>' +
      current_balance_str +
      "</span></div>";
    content += "</div>";
  ...
  $(".amortization-column-outer").each(function () {
    const total_principal = 0;
    const total_interest_yearly = 0;
    const total_payment_yearly = 0;
    const total_balance_yearly = 0;
    $(this)
      .find(".amortization-column-inner .amortization-column")
      .each(function () {
        //principal
        const principal_int = $(this).find(".principal span").html();
        const new_principal_int = parseInt(principal_int.replace(/,/g, ""));
        total_principal += new_principal_int;

        //interest
        const interest_int = $(this).find(".interest span").html();
        const new_interest_int = parseInt(interest_int.replace(/,/g, ""));
        total_interest_yearly += new_interest_int;

        // total payment
        const total_payment = $(this).find(".total-payment span").html();
        const new_total_payment_int = parseInt(total_payment.replace(/,/g, ""));
        total_payment_yearly += new_total_payment_int;

        // balance
        const balance_int = $(this).find(".balance span").html();
        const new_balance_int = parseInt(balance_int.replace(/,/g, ""));
        total_balance_yearly += new_balance_int;
      });
    const total_principal_str = total_principal
      .toFixed(0)
      .toString()
      .replace(/,/g, "")
      .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    const total_interest_str = total_interest_yearly
      .toFixed(0)
      .toString()
      .replace(/,/g, "")
      .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    const total_payment_str = total_payment_yearly
      .toFixed(0)
      .toString()
      .replace(/,/g, "")
      .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    const total_balance_str = total_balance_yearly
      .toFixed(0)
      .toString()
      .replace(/,/g, "")
      .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    ...
    $(this)
      .find(".principal-sum")
      .html("$<span>" + total_principal_str + "</span>");
    $(this)
      .find(".interest-sum")
      .html("$<span>" + total_interest_str + "</span>");
    $(this)
      .find(".total-payment-sum")
      .html("$<span>" + total_payment_str + "</span>");
    $(this)
      .find(".balance-sum")
      .html("$<span>" + total_balance_str + "</span>");
    ...
    tooltip: {
      formatter: function () {
        y_str = this.y
          ...
          .replace(/,/g, "")
          .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
        stacktotal_str = this.point.stackTotal
          .toFixed(0)
          .toString()
          .replace(/,/g, "")
          .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
